...
function Chat(props){
  //УЛЬТРА костыль думаю! 44 страка
let k=0;
...
<div  className="center">
  {props.MessageX.map((text)=>{
    if(text.id[0]===props.onClick.map((todo)=>{return todo.id})[0]){
      return <p key= {k= k+1}>{text.Name}:<br/>{text.message}</p>
    }else{
       ...
       <div className="">
         <div className=""> 
           <textarea  autoFocus className="Wrapper4" type="text" placeholder="Напишите сообщение..." rows="3" ref={textInput}></textarea>
            <input   onClick={()=>{props.ReadMessange(textInput.current.value,props.onClick);Show();}} type="button" value="Отправить"></input>
         </div>
         <div className="">
